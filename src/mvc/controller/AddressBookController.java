...
import mvc.views.SearchDialog;
import mvc.views.SearchPanel;
...
public class AddressBookController {
    ...
    private void openAdd(){   
       view.getMainGUIFrame().setVisible(false);
        ...
        choiceListener = new ActionListener() {
            ...
            //callDAO("Add");
            Boolean flag = false,testFlag=false;
   ...
   */
    ...
    public Boolean mobileValidate()
    {
        boolean test=false;
        String mobile = contactDetailsPanel.getMobileField().getText();
        if(mobile.length()<10 || mobile.length()>10)//mobile.length() < 10)|| mobile.length() > 10)
        {
            JOptionPane.showMessageDialog(new JFrame(), "Mobile Number should contain 10 numbers only","Inane error", JOptionPane.ERROR_MESSAGE);
        }
        else
        {
            test=true;
    ...
    public void search(String selectedName)
    {
        view.getMainGUIFrame().setVisible(false);
         sdialog =  new SearchDialog("Enter the Name you want to search");    
         sdialog.getPanel().setLocation(500,500);
        choiceListener = new ActionListener() {
           @Override
            public void actionPerformed(ActionEvent e) {
            if(e.getSource()== sdialog.getSubmitButton())
            {
            contactDetailsPanel=dialog.getPanel();
            //callDAO("Add");
            /*Boolean flag = false;
